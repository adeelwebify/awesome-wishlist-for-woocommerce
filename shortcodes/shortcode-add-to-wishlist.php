...
function aws_single_product_wishlist_button():string {
  ...
  // Check if the current post is of type 'product' and if it's a single product page
  if (get_post_type($post) !== 'product') {
    return ''; // Return nothing if not a product post type
  }
  
  $product_id = $post->ID;
  $is_single_product = is_singular( 'product' );
 
  // Button HTML for Single Product Page
  return sprintf(
    ...
    $is_single_product ? 'single' : 'archive',
    esc_attr( $product_id )
  );
}
